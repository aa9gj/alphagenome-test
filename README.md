# Prioritizing Causal Variants at the FHL3 Estimated BMD GWAS Locus with AlphaGenome
...
Starting from GWAS summary statistics and a lead SNP, the pipeline identifies LD proxies, lifts coordinates to GRCh38, runs AlphaGenome variant effect predictions across all GTEx tissues and output modalities, and produces ranked variant prioritization tables suitable for downstream functional follow-up.
...
| 03 | `03_z_score_within_modality.py` | Compute robust (MAD-based) and standard z-scores of AlphaGenome effect sizes within each modality, enabling cross-modality comparison of variant impact. |
| 04 | `04_score_variant.py` | Run AlphaGenome's built-in `RECOMMENDED_VARIANT_SCORERS` via `score_variant()` to produce calibrated per-gene and per-track effect scores comparable across modalities. |
| 05 | `05_rank_prioritization.py` | Aggregate variant scorer rankings into overall SNP prioritization: per-scorer rankings, cross-scorer consensus ranking, and tissue-specific rankings. |
| 06 | `06_collapse_eqtl_sqtl.py` | Collapse scores to per-SNP-per-gene tables for eQTL-like (RNA-seq `GeneMask` scorers) and sQTL-like (splicing scorer/output) signals. |
...
Steps 02 and 04 call the AlphaGenome API and require an API key set as an environment variable:
...
| `alphagenome` | 02, 04 (AlphaGenome API client and variant scorers) |
...
# 4. Z-score effects within each modality
python 03_z_score_within_modality.py

# 5. Score variants with AlphaGenome recommended scorers
python 04_score_variant.py

# 6. Rank and prioritize SNPs
python 05_rank_prioritization.py

# 7. Collapse to eQTL/sQTL-like per-SNP-per-gene tables
python 06_collapse_eqtl_sqtl.py
```

Steps 02 and 04 include **checkpoint/resume support** -- if interrupted, re-running the script will skip already-completed variant-tissue pairs.
...
| `alphagenome_allGTEx_allOutputs_COLLAPSED_zscores.csv` | 03 | Z-scored effect sizes |
| `qc_modality_counts.csv` | 03 | QC table of per-modality counts and statistics |
| `variant_scores_tidy.csv` | 04 | Tidy scores from recommended variant scorers |
| `snp_prioritization_by_scorer.csv` | 05 | Per-SNP rankings within each scorer |
| `snp_prioritization_overall.csv` | 05 | Consensus SNP ranking across all scorers |
| `snp_prioritization_by_tissue.csv` | 05 | Tissue-specific SNP rankings |
| `variant_scores_per_snp_gene_eqtl.csv` | 06 | Per-SNP-per-gene eQTL-like scores |
| `variant_scores_per_snp_gene_sqtl.csv` | 06 | Per-SNP-per-gene sQTL-like scores |

## References

1. Cheng, J., Novati, G., Pan, J. *et al.* Accurate proteome-wide missense variant effect prediction with AlphaMissense. *Nature* (2025). https://doi.org/10.1038/s41586-025-10014-0
